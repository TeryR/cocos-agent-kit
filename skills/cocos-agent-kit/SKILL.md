...
## 血泪坑位清单(两个真实会话实测,违反必翻车)

1. **UI 坐标系:Canvas 锚点 0.5 → 子节点 position 原点在画布中心**(1280×720 画布 x∈[-640,640] y∈[-360,360]),不是左下角!屏幕/触摸事件坐标才是左下原点——两边换算要 ±半宽半高。世界坐标(scene_tree 的 worldPosition)是左下原点,与 position 不同,别混用;
2. **预览读磁盘、act 改内存**:预览前必须 `save_scene`;场景保存后预览页要手动刷新(F5),不刷新看到的是旧场景——"预览里没有"先想是不是没保存/没刷新,别急着改场景;
3. **纯逻辑游戏别挂物理组件**:RigidBody2D/Collider2D 与脚本逐帧 setPosition 会打架(重力+逻辑叠加成斜飞、瞬移穿屏、碰撞回调时序错乱)。自写位置积分+AABB 数学碰撞最稳;
4. **新项目没有一张贴图**:内置单色图 uuid `7d8f9b89-4fd1-4c9f-a3ab-38ec7cded7ca@f9941`(全项目通用,无需导入)——Sprite 赋它再 color 染色即得任意色块。别自己生成 PNG、别从别的项目拷贝;
5. **内置组件名带 `cc.` 前缀**:`act_remove_component` 传裸名 `RigidBody2D` 会静默失败,必须 `cc.RigidBody2D`;摘除后组件列表短暂显示"还在"是 destroy 延迟,2 秒后复查;
6. **脚本必须写到游戏项目绝对路径** `assets/` 下(不是工作区目录!),写完 `refresh_assets` 等编译,再用 `@ccclass` 名或脚本 uuid 挂载;
7. **改脚本默认值不改变已挂节点的序列化值**:改了 @property 默认后,要把已挂实例的属性也设一遍;
8. **逻辑容器节点别挂 Sprite**(看不见就对了;挂了反而多一个可见块);
9. **预览调试**:浏览器 Console 你看不到——把调试信息写到场景里的 Label(调试 HUD 模式)是最可靠的运行时反馈;后台 tab 会被节流到 1fps,判运动必须前台;
10. **编辑器忙态时 execute-scene-script 间歇失败**:等 2 秒重试即可,不是代码错;多项目并行时先确认 7420 归属(netstat 查监听进程)。
